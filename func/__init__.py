...
def save_image(form_picture):
    ...
    picture_path = os.path.join(
        current_app.root_path, "static\\images\\profile", picture_fn
    )
...
def get_all_admins(page, per_page, email, is_super_admin, fullname, phone):
    try:
        ...
        paginated_admins = admins.order_by(Admin.date.desc()).paginate(
...
def get_all_users(
    ...
    try:
        ...
        paginated_users = users.order_by(User.date_joined.desc()).paginate(
            page=page, per_page=per_page, error_out=False
        )
...
def get_user_transactions(page, per_page, transaction_type, status, category, user):
    try:
        ...
        outflow_transactions = transactions.filter(
            Transaction.transaction_type == "DBT"
        ).count()
        if transaction_type:
        ...
        paginated_transactions = transactions.order_by(
            Transaction.date_posted.desc()
...
def get_all_transactions(
    ...
    try:
        ...
        if bank_name:
            transactions = transactions.filter(
                Transaction.bank_name.ilike(f"%{bank_name}%")
            )
        if receiver:
            transactions = transactions.filter(
                Transaction.receiver.ilike(f"%{receiver}%")
            )
        paginated_transactions = transactions.order_by(
            Transaction.date_posted.desc()
        ).paginate(page=page, per_page=per_page, error_out=False)
...
def statistics():
    ...
    try:
        ...
        transaction_stats = {
            ...
            "success": safe_first(
                Transaction.query.filter(Transaction.status == "Success")
                .with_entities(func.count(), func.sum(Transaction.transaction_amount))
                .first()
            ),
            "pending": safe_first(
                Transaction.query.filter(Transaction.status == "Pending")
                .with_entities(func.count(), func.sum(Transaction.transaction_amount))
                .first()
            ),
            "failed": safe_first(
                Transaction.query.filter(Transaction.status == "Failed")
                .with_entities(func.count(), func.sum(Transaction.transaction_amount))
                .first()
            ),
            "refund": safe_first(
            ...
            "outflow": safe_first(
                Transaction.query.filter(Transaction.transaction_type == "DBT")
                .with_entities(func.count(), func.sum(Transaction.transaction_amount))
                .first()
            ),
        }
        ...
        for category in categories:
            count, amount = db.session.query(
                func.count(), func.sum(Transaction.transaction_amount)
            ).join(
                TransactionCategories, Transaction.category == TransactionCategories.id
            ).filter(
                TransactionCategories.category == category
            ).first() or (
                0,
                0,
            )
            category_stats[category] = {"count": count, "amount": amount or 0}
