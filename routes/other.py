...
def pay():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        transact1 = Transaction(transaction_type='CRT', transaction_amount=amount,
                                sender=current_user.username, user_id=user1.id)
        db.session.add(transact1)
        db.session.commit()

        current_user.account_balance -= amount
        db.session.commit()
        transact2 = Transaction(transaction_type='DBT', transaction_amount=amount,
                                sender=user1.username, user_id=current_user.id)
